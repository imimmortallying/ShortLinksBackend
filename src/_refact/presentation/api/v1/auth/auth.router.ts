  ...
  async (req: RequestWithBody<SignInDto>, res: Response) => {
    ...
    if (foundUser._tag === 'Right') {
      return res
        ...
        .json({ message: foundUser.right });
    } else {
      return res.status(StatusCodes.CONFLICT).json({ message: foundUser.left });
    }
  }
);
...
authRouter.post(
  ...
  async (req: RequestWithBody<SignInDto>, res: Response) => {
    ...
    return foundUser._tag === 'Left'
      ...
      : res
          .cookie('refreshToken', foundUser.right.refreshToken, {
            maxAge: 30 * 24 * 60 * 60 * 1000,
            httpOnly: true,
          })
          .json({
            accessToken: foundUser.right.accessToken,
            user: { username: req.body.username },
          }) //в прошлой версии я возвращал еще и username. Сопоставить с фронтом
...
authRouter.delete('/signout', async (req: Request, res: Response) => {
  ...
  return deletedSession._tag === 'Left'
    ? res
        ...
        .json({ message: deletedSession.left })
    : res
...
authRouter.get('/refresh', async (req: Request, res: Response) => {
  ...
  return newSession._tag === 'Right'
    ? res
        .cookie('refreshToken', newSession.right.refreshToken, {
          maxAge: 30 * 24 * 60 * 60 * 1000,
          httpOnly: true,
        })
        .json({
          accessToken: newSession.right.accessToken,
          user: { username: newSession.right.username },
        })
        .status(StatusCodes.OK)
